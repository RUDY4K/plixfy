 ...
 */
// GameDistribution requires the gd_sdk_referrer_url query param so their
// SDK reports impressions back to our publisher account (without it the
// games still play but we don't earn anything — and they may rate-limit
// unattributed traffic). The hardcoded www.plixfy.com is intentional: it
// must match the domain registered in our GD publisher dashboard, not
// $NEXT_PUBLIC_SITE_URL (which is localhost in dev).
const gd = (id: string) =>
  \`https://html5.gamedistribution.com/\${id}/?gd_sdk_referrer_url=https://www.plixfy.com\`;
