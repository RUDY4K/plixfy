...
/** Catchy auto-generated names so a new visitor never sees an empty profile. */
const NAME_PREFIXES = [
 ...
 * until the underlying value changes. We achieve that by caching the
 * parsed value alongside the raw string; a new parse only happens when
 * the string differs.
 */
...
/** Trim + cap nickname. Empty / whitespace-only is rejected (returns false). */
...
/** A funky random default — used to pre-fill the nickname modal. */
