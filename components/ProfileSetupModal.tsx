...
export default function ProfileSetupModal({ open, initial, onClose, onSaved }: ProfileSetupModalProps) {
  ...
  // Re-seed defaults each time the modal opens fresh (no `initial`).
  useEffect(() => {
    if (!open) return;
    if (!initial) {
  ...
  // ESC closes the modal so it doesn't trap keyboard users.
  useEffect(() => {
    if (!open) return;
        ...
        <label className="mb-3 block">
          ...
          <div className="flex gap-2">
            <input
              ...
              autoCapitalize="off"
              spellCheck={false}
              className="flex-1 rounded-lg border border-neutral-800 bg-neutral-900 px-3 py-2 text-sm text-white placeholder:text-neutral-600 focus:border-emerald-500 focus:outline-none"
              ...
              return (
                <button
                  ...
                  className={`flex aspect-square items-center justify-center rounded-lg border text-xl transition ${
                    selected
                      ? 'border-emerald-500 bg-emerald-500/15 scale-110'
