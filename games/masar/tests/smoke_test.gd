...
func _run() -> void:
    ...
    assert(game.get_level_color_for_test(1) == Color("#315CF5"), "The first level must use the cobalt theme")
    ...
    await process_frame
    assert(board.TILE_COLORS.size() == 1, "Every stage must use one signature color family")
